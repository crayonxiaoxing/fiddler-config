...
class Handlers
{
    ...
    */
    ...
    static var s_international = "国际随机IP 关闭";
    static var s_domestic = "国际随机IP 关闭";

    Fiddler.QuickLinkMenu("Plugins")
    Fiddler.QuickLinkItem('国内随机IP','国内随机IP')
    Fiddler.QuickLinkItem('国际随机IP','国际随机IP')
    public static function DoLinksMenu(sText: String, sAction: String)
    {
        switch(sAction)
        {
            case "国际随机IP":
            {
                if(s_international == "国际随机IP 开启")
                {
                    s_international = "国际随机IP 关闭";
                }else{
                    s_international = "国际随机IP 开启";
                }
                // s_international = "国际随机IP 开启"?"国际随机IP 关闭":"国际随机IP 开启";
                MessageBox.Show(s_international);
                break;
            }

            case "国内随机IP":
            {
                if(s_domestic == "国内随机IP 开启")
                {
                    s_domestic = "国内随机IP 关闭";
                }else{
                    s_domestic = "国内随机IP 开启";
                }
                // s_domestic = "国内随机IP 开启"?"国内随机IP 关闭":"国内随机IP 开启";
                MessageBox.Show(s_domestic);
            }
        }

        // FiddlerObject.log(s_international);
        // FiddlerObject.log(s_domestic);
    }

    public static function randomInternational()
    {
        return int(Math.random()*223)+'.'+int(Math.random()*253)+'.'+int(Math.random()*253)+'.'+int(Math.random()*253);
    }         

    public static function randomDomestic()
    {
        return int(61)+'.'+int(128+ int(Math.random()*61))+'.'+int(Math.random()*253)+'.'+int(Math.random()*253);
    }
    ...
    static function OnBeforeRequest(oSession: Session) {
        if(s_international == "国际随机IP 开启")
        {
            var ip_international = randomInternational();
            oSession.oRequest["X-Forwarded-For"] = ip_international;
            oSession.oRequest["Client-IP"] = ip_international;
            oSession.oRequest["X-Real-IP"] = ip_international;
            oSession.oRequest["Cluster-Client-IP"] = ip_international;
        }

        if(s_domestic == "国内随机IP 开启")
        {
            var ip_domestic = randomDomestic();
            oSession.oRequest["X-Forwarded-For"] = ip_domestic;
            oSession.oRequest["Client-IP"] = ip_domestic;
            oSession.oRequest["X-Real-IP"] = ip_domestic;
            oSession.oRequest["Cluster-Client-IP"] = ip_domestic;
        }
    ...
    */
    ...
    static function OnPeekAtResponseHeaders(oSession: Session) {
        // FiddlerApplication.UI.miViewWide.Text = "上下显示模式";
        // FiddlerApplication.UI.miViewDefault.Text = "左右显示模式";
        // FiddlerApplication.UI.miViewStacked.Text = "堆叠显示模式";
        // FiddlerApplication.UI.mnuTools.Text = "设置";
        // FiddlerApplication.UI.miRulesRemoveEncoding.Text = "自动解码";
        // FiddlerApplication.UI.pageInspector.Text = "抓包工具";
        // FiddlerApplication.UI.pageStatistics.Text = "数据统计";
        // FiddlerApplication.UI.pageResponder.Text = "自动响应";
        // FiddlerApplication.UI.pageBuilder.Text = "模拟发包";
        // FiddlerApplication.UI.mnuRules.Text = "规则";
        // FiddlerApplication.UI.miManipulateIgnoreImages.Text = "隐藏图片链接";
        // FiddlerApplication.UI.miRulesIgnoreConnects.Text = "隐藏服务连接";
        // FiddlerApplication.UI.miRulesBreakAtRequest.Text = "拦截发送数据";
        // FiddlerApplication.UI.miRulesBreakAtResponse.Text = "拦截返回数据";
        // FiddlerApplication.UI.miSessionReplay.Text = "重新发送";
        // FiddlerApplication.UI.miSessionCopy.Text = "复制";
        // FiddlerApplication.UI.miManipulateGZIP.Text="解压GZIP";
        // FiddlerApplication.UI.miSessionSave.Text ="保存";
        // FiddlerApplication.UI.miSessionSelect.Text ="选择"

        
        // 限速  每上传1KB延迟300ms , 勾选rule-performance-Simulate Modem Speeds生效
        // oSession["request-trickle-delay" ] = "300";

        // 如果需要修改返回请先修改这条代码中的完整URL，不带http或https
        // if (oSession.HostnameIs(url))
        // { 
